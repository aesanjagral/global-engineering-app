...
class ViewDialog(QDialog):
    ...
    def init_ui(self):
        ...
        remark_label.setStyleSheet(label_style)
        remark_value = QLabel(self.entry.get("Remark", ""))
        remark_value.setStyleSheet(value_style)
        remark_value.setWordWrap(True)
        work_layout.addWidget(remark_label)
        work_layout.addWidget(remark_value)
        ...
        # Set the scroll area as the dialog's main layout
...
class EditDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Plot No.
        self.plot_no = QLineEdit(self.entry.get("Plot No.", ""))
        ...
        """)
        location_layout.addRow("Plot No.", self.plot_no)
    ...
    def save_changes(self):
        ...
        self.entry["Plot No."] = self.plot_no.text().strip()
        ...
        # Format Party Address in the same way as add_entry.py
        party_address_lines = []
...
class ReportModule(QWidget):
    def __init__(self):
        ...
        self.user_id = os.getlogin()  # Get current Windows username as user_id
    ...
    def edit_entry(self, file_no):
        ...
        try:
            # Try to acquire case-specific lock
            if not github_sync.acquire_case_lock(file_no, self.user_id):
                QMessageBox.warning(self, "Warning", f"Case {file_no} is currently being edited by another user. Please try again later.")
                return False
            ...
            if entry:
                ...
                if dialog.exec_() == QDialog.Accepted:
                    ...
                    # Release case-specific lock
                    github_sync.release_case_lock(file_no)
                    return True
            return False
        except Exception as e:
